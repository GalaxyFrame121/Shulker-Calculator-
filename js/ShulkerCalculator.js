document.addEventListener("DOMContentLoaded", function () {
    ...
    btn.addEventListener("click", function () {
        ...
        const FullShulk = Math.floor(num / 27);
        const Remaining = num % 27;

        const totalShulks = Remaining === 0 ? FullShulk : FullShulk + 1;

        if (Remaining === 0) {
            result1.textContent = t.resultText + totalShulks + " " + t.shulkerName;
            result2.textContent = "";
            img.classList.remove("hidden");
        }
        else {
            const Slots = 27 - Remaining;

            result1.textContent = t.resultText + totalShulks + " " + t.shulkerName;
            result2.textContent = t.decimalText + Slots + " " + t.remain;
            img.classList.remove("hidden");
        }
    })
